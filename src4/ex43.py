...
class CentralCorridor(Scene):
    ...
    def enter(self):
        ...
        if action == "발사!":
            ...
            print "망가지자 광기 어린 분노에 빠저들어 당신이 죽을 때까지 얼굴을 쏘아"
            ...
            print "동안 발이 미끄러져 금속 벽에 머리를 치고 기적합니다. 잠시 후 정신을"
            print "차리지만 고던인에게 머리를 짓밟혀 죽고 잡아 먹힙니다."
            return 'death'

        elif action == "농담하기":
            print "운 좋게도 당신은 학교에서 고던어 욕설을 배웠습니다. 이는 고던 농담을"
            print "하나 합니다."
            print "Lbhe zbgure vf fb sng, jura fur fvbf nebhaq gur ubhfr, fur fvg nebhaq gur ubhfr."
...
class LaserWeaponArmory(Scene):
    ...
    def enter(self):
        print "당신은 무기고로 뛰어 구러 들어가 쪼그려 앉고 방을 살피며 숨어 있을지도"
        print "모르는 고던인을 찾습니다. 쥐죽은 듯이, 지나치게 조용합니다."
        print "일어서서는 문 건너편으로 달려가 컨테이너에서 중선자탄을 찾습니다."
        print "상장는 비밀번호로 잠겨 있고 폭탄을 꺼내려면 비밀번호를 알아내야만"
...
class TheBridge(Scene):
    ...
    def enter(self):
        ...
        print "빼앗으려던 고던인 5며을 놀라게 합니다. 그 모두가 방금 전에 본"
        print "고던인보다 더 못생긴 광대 복장을 하고 있습니다. 아직 무기를 뽑지는"
        print "않았지만 당신이 팔에 낀 활성화된 폭탄을 보고 터뜨리고 않고 싶어"
        print "합니다."

        action = raw_input(">")

        if action == "폭탄 던지기":
            print "당신은 당황해서 고던인 무리에게 폭탄을 집어 던지고 문으로 펄쩍"
            print "뛰어 오릅니다. 폭탄을 놓자마자 고던인이 뒤에서 당신을 쏴 맞추어"
            print "죽여버립니다."
            print "죽어가는 동안 다른 고전인이 미친듯이 폭탄을 해체하려는 것을"
            print "봅니다. 당신은 폭탄이 터지면 고던인 모두가 터져 나가리라고"
            print "깨달으며 죽습니다."
            return 'death'

        elif action == "천천히 폭탄 설치하기":
            print "팔에 낀 폭탄을 광선총으로 겨누자 고던인들은 팔을 들고 땀을 흘리기"
            print "시작합니다. 당신은 문 뒤에 바짝 붙어 열고 광선총을 겨눈 채로"
            print "조심스럽게 폭탄을 바닥에 설치합니다."
            print "다음으로 문 밖으로 뛰쳐나와 닫기 버튼을 두드리고 잠금장치를"
            print "쐬버려 고던인들이 나올 수 없게 만들어버립니다."
...
class EscapePod(Scene):
    ...
    def enter(self):
        ...
        print "합니다. 이 중 몇개는 손상되었을 수 있지만 살펴볼 시간이 없습니다."
        ...
        if int(guess) != good_pod:
            ...
            print "구명정은 우주의 진고으로 빠져나가고, 콩깍지가 터지듯 안으로"
            print "무너지며 당신을 곤약처럼 으스러뜨립니다."
            return 'death'

        else:
            print "%번 구명정으로 뛰어들어 탈출 단추를 누릅니다." % guess
            print "구명정은 가볍게 우주로 미끄러져 나가며 아래의 행성으로 향합니다."
            print "행서으로 날아가는 동안 뒤돌아보니 우주선이 파괴되면서 밝은 별처럼"
            print "폭발하고 고던 우주선가지 없애버리는 것을 확인합니다."
            print "당신이 이겼습니다."
            return 'finished'
...
class Engine(object):
    ...
    def __init__(self, scene_map):
        print "엔진 __init__의 scene", scene_map
